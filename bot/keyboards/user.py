...
def main_menu_kb() -> InlineKeyboardMarkup:
    ...
    builder.row(InlineKeyboardButton(
        text=f"{plain(CATALOG)} Каталог товаров",
        callback_data="catalog"
    ))
    builder.row(
        InlineKeyboardButton(text=f"{plain(CART)} Корзина", callback_data="cart"),
        InlineKeyboardButton(text=f"{plain(BAG)} Мои заказы", callback_data="my_orders"),
    )
    builder.row(
        InlineKeyboardButton(text=f"{plain(PROMO)} Промокод", callback_data="promo"),
        InlineKeyboardButton(text=f"{plain(SUPPORT)} Поддержка", callback_data="support"),
    )
    builder.row(InlineKeyboardButton(text=f"{plain(PROFILE)} Профиль", callback_data="profile"))
...
def product_kb(product_id: int, stock: int) -> InlineKeyboardMarkup:
    ...
    if stock > 0:
        builder.row(InlineKeyboardButton(
            text=f"{plain(CART)} Добавить в корзину",
            callback_data=f"add_cart_{product_id}"
        ))
    else:
        builder.row(InlineKeyboardButton(
            text=f"{plain(FAIL)} Нет в наличии",
...
def cart_kb(has_items: bool) -> InlineKeyboardMarkup:
    ...
    if has_items:
        builder.row(InlineKeyboardButton(
            text=f"{plain(OK)} Оформить заказ",
            callback_data="checkout"
        ))
        builder.row(InlineKeyboardButton(
            text=f"{plain(FAIL)} Очистить корзину",
            callback_data="clear_cart"
        ))
    builder.row(InlineKeyboardButton(text=f"{plain(BACK)} В меню", callback_data="main_menu"))
    return builder.as_markup()
...
def payment_method_kb(order_id: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(
        text=f"{plain(SHIELD)} Оплата криптовалютой (CryptoBot)",
        callback_data=f"pay_crypto_{order_id}"
    ))
    builder.row(InlineKeyboardButton(
        text=f"{plain(LOCK)} RollyPay (скоро)",
...
def payment_link_kb(pay_url: str, order_id: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text=f"{plain(SHIELD)} Оплатить сейчас", url=pay_url))
    builder.row(InlineKeyboardButton(
        text=f"{plain(REFRESH)} Проверить оплату",
        callback_data=f"check_payment_{order_id}"
    ))
    builder.row(InlineKeyboardButton(
        text=f"{plain(FAIL)} Отменить",
        callback_data=f"cancel_order_{order_id}"
    ))
    return builder.as_markup()
...
def back_to_menu_kb() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text=f"{plain(BACK)} В главное меню", callback_data="main_menu"))
    return builder.as_markup()
