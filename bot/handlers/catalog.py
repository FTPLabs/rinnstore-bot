from decimal import Decimal
  ...
  def _get_back_cb(category) -> str:
      """Вычисляет callback кнопки Назад для категории."""
      if not category.parent_id:
          return "catalog"
      # Родитель существует — используем subcat_ (cb_subcat умеет обрабатывать любой уровень)
  ...
  # === Клик по подкатегории (любого уровня вложенности) ===
  ...
  async def cb_subcat(call: CallbackQuery, session: AsyncSession, user: User):
      ...
      # FIX: проверяем наличие подкатегорий — если есть, показываем их
      # Ранее cb_subcat сразу переходил к товарам, игнорируя вложенные подкатегории
      subcats = await get_subcategories(session, cat_id)
  ...
  async def cb_cat_back(call: CallbackQuery, session: AsyncSession, user: User):
      ...
      product_list = await get_products_in_category(session, cat_id)
      icon = "📁" if category.parent_id else "📂"
      await call.message.edit_text(
