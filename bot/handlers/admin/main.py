...
async def cmd_admin(message: Message, session: AsyncSession, user: User):
    ...
    await message.answer(stats_text(stats), reply_markup=admin_main_kb())
...
async def cb_admin_main(call: CallbackQuery, session: AsyncSession, user: User):
    ...
    stats = await get_stats(session)
    await call.message.edit_text(stats_text(stats), reply_markup=admin_main_kb())
    await call.answer()
...
async def cb_admin_stats(call: CallbackQuery, session: AsyncSession, user: User):
    ...
    await call.message.edit_text(stats_text(stats), reply_markup=builder.as_markup())
    await call.answer()
