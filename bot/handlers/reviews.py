...
async def rate_review(call: CallbackQuery, session: AsyncSession, user: User, state: FSMContext):
    ...
    review.status = "awaiting_anonymity"
    review.rated_at = datetime.now(timezone.utc)
    await session.commit()
    await call.message.answer("Хотите оставить отзыв анонимно?", reply_markup=anonymity_kb(review.id))
    await call.answer()


@router.callback_query(F.data.regexp(r"^review_anonymous_\d+_[01]$"))
async def choose_anonymity(call: CallbackQuery, session: AsyncSession, user: User, state: FSMContext):
    parts = call.data.split("_")
    review_id, anonymous = int(parts[2]), bool(int(parts[3]))
    result = await session.execute(select(Review).where(Review.id == review_id).with_for_update())
    review = result.scalar_one_or_none()
    if not review or review.user_id != user.id or review.status != "awaiting_anonymity":
        await call.answer(t(user, "review_unavailable"), show_alert=True)
        return
