        ...
        async with AsyncSessionFactory() as session:
            ...
            if processed:
                ...
                if order_id_str and str(order_id_str).isdigit():
                    ...
                    await _notify_user_webhook(session, order_id, request.app.get("bot"))
...
async def _notify_user_webhook(session: AsyncSession, order_id: int, bot: Bot | None):
    """
    Уведомление пользователя при оплате через webhook.
    Вызывает deliver_order (имеет защиту от двойной выдачи).
    """
    if not bot:
        return
    try:
        ...
        if order.status not in ("paid", "delivered", "partial"):
            logger.warning(f"Webhook notify: заказ #{order_id} в статусе {order.status!r} — пропускаем")
            return

        delivered = await deliver_order(session, order_id)
        if not delivered:
            logger.warning(f"Webhook notify: deliver_order вернул пусто для заказа #{order_id}")
            return
