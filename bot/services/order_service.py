import logging
  ...
  REFERRAL_BONUS_PERCENT = Decimal("5")  # 5% от суммы заказа
  ...
  async def deliver_order(session: AsyncSession, order_id: int) -> list[dict]:
      ...
      # FIX: with_for_update() на DeliveredItem — исключает race condition при двойном вызове
      ...
      # FIX: total_spent обновляется всегда — деньги уже списаны при любом исходе
      await session.execute(
          update(User)
      ...
      # FIX: реферальный бонус — 5% рефереру при первом успешном заказе
      if not out_of_stock:
          ...
          if buyer and buyer.referred_by:
              ...
              if prev_delivered == 0:
                  ...
                  logger.info(f"Реферальный бонус {bonus} ₽ начислен пользователю {buyer.referred_by}")
